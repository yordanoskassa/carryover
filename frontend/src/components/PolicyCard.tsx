    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-4 py-2.5 border-b bg-card/50">
        ...
        {policy.ai_structured && (
          <span className="inline-flex items-center gap-1 text-[9px] font-mono text-primary bg-primary/10 px-1.5 py-0.5 rounded shrink-0">
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        {policy.key_requirements.length > 0 && (
          <div>
            <h5 className="text-[10px] font-mono font-semibold text-muted-foreground uppercase mb-1.5">Requirements</h5>
        ...
        {policy.documents.length > 0 && (
          <div>
            <h5 className="text-[10px] font-mono font-semibold text-muted-foreground uppercase mb-1.5 flex items-center gap-1">
              <FileText size={11} /> Documents needed
            </h5>
            <div className="flex flex-wrap gap-1.5">
              {policy.documents.map((d, i) => (
                <span key={i} className="text-[11px] bg-muted px-2 py-0.5 rounded text-foreground/90">{d}</span>
              ))}
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <ol className="space-y-1">
              {policy.steps.map((s, i) => (
                <li key={i} className="text-xs flex items-start gap-2">
                  <span className="w-4 h-4 rounded-full bg-primary/15 text-primary text-[9px] font-bold flex items-center justify-center shrink-0 mt-0.5 tabular-nums">{i + 1}</span>
                  <span>{s}</span>
                </li>
              ))}
            </ol>
